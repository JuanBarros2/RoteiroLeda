...
public class StudentTestHashtableOpenAddressLinearProbing {
	...
	public void testSize() {
		...
		for (int i = 1; i <= PROPOSED_SIZE; i++) {
			assertEquals(i - 1, table2.size());
			table2.insert(new HashtableElement(i));
			assertEquals(i, table2.size());
		}

		for (int i = PROPOSED_SIZE; i != 0; i--) {
			assertEquals(i, table2.size());
